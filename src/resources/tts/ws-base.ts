...
import { TTSEmitter, TTSStreamMessage, WebSocketError, buildURL } from './internal-base';
...
export interface TTSWSBaseOptions {
  /**
   ...
   */
  reconnect?: TTSWSReconnectOptions | null | undefined;
...
export abstract class TTSWSBase extends TTSEmitter {
  url: URL | undefined;
  ...
  private _connect(): WebSocketLike {
    ...
    socket.on('message', (data: string | ArrayBuffer | ArrayBufferView, isBinary: boolean) => {
      ...
      try {
        event = JSON.parse(text) as TTSAPI.WebsocketResponse;
      } catch {
        this._emit('raw', data);
        return;
      }
