...
export default class Websocket {
    ...
    constructor(
        ...
        private readonly options: Tts.Options
    ) {
    ...
    send(inputs: WebSocketTtsRequest, { timeout = 0 }: WebSocketStreamOptions = {}) {
		...
		this.socket?.send(JSON.stringify(serializers.WebSocketTtsRequest.jsonOrThrow(inputs, { unrecognizedObjectKeys: "strip" })));
        ...
        this.socket = new ReconnectingWebSocket(
            async () => {
                const baseUrl =
                    ...
                    environments.CartesiaEnvironment.Production).replace(/^https?:\/\//, "");
                const params = {
